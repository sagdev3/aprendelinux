...
function rateLimit(req, key, { limit = 8, windowMs = 15 * 60 * 1000 } = {}) {
...
function validateCsrf(req, res) {
  if (!unsafeMethods.has(req.method) || req.url === "/api/csrf") return true;
...
async function handleApi(req, res, pathname, pool) {
  ...
  try {
    ...
    if (req.method === "POST" && pathname === "/api/register") {
      if (!rateLimit(req, "register")) return json(res, 429, { error: "Demasiados intentos. Intenta más tarde." });
      ...
      try {
        ...
        sessions.set(token.split(".")[0], result.insertId);
    ...
    if (req.method === "POST" && pathname === "/api/login") {
      if (!rateLimit(req, "login")) return json(res, 429, { error: "Demasiados intentos. Intenta más tarde." });
      const body = await readBody(req);
      const email = normalizeEmail(body.email);
      const password = String(body.password || "");
      ...
      const token = makeSessionToken();
      sessions.set(token.split(".")[0], user.id);
      setSessionCookie(res, token);
