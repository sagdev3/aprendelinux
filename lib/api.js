...
function isPostgres(pool) {
  return pool?.dialect === "postgres";
}
    ...
    await pool.execute(
      `INSERT INTO rate_buckets (bucket_key, count, reset_at)
       ...
       ON CONFLICT (bucket_key) DO UPDATE SET
        ...
        reset_at = CASE WHEN rate_buckets.reset_at <= NOW() THEN EXCLUDED.reset_at ELSE rate_buckets.reset_at END`,
      [bucketKey, resetAt]
    );
  } else {
    await pool.execute(
...
function createPostgresPool() {
  ...
  try {
    ...
    throw new Error("Falta instalar la dependencia pg. Ejecuta npm install antes de iniciar el servidor.");
  }
  const pool = new pg.Pool({
    ...
    ssl: process.env.PGSSL === "false" ? false : process.env.DATABASE_URL ? { rejectUnauthorized: false } : undefined,
    max: 10
  });
  return {
    dialect: "postgres",
    async execute(sql, params = []) {
      let index = 0;
      const text = sql.replace(/\?/g, () => `$${index += 1}`);
    ...
    if (req.method === "POST" && pathname === "/api/register") {
      ...
      try {
        const [result] = await pool.execute(
          isPostgres(pool)
            ? "INSERT INTO users (name, email, password_hash, password_salt) VALUES (?, ?, ?, ?) RETURNING id"
        ...
        await pool.execute(
          isPostgres(pool)
            ? "INSERT INTO user_progress (user_id, active_module, mode, done_modules) VALUES (?, 0, 'kid', '[]'::jsonb)"
        ...
        return json(res, 201, { user: { id: userId, name: name.slice(0, 80), email } });
      } catch (error) {
        if (error.code === "ER_DUP_ENTRY" || error.code === "23505") return json(res, 409, { error: "Ese email ya está registrado" });
        throw error;
      }
    }
        ...
        isPostgres(pool)
          ? `INSERT INTO user_progress (user_id, active_module, mode, done_modules)
             ...
             ON CONFLICT (user_id) DO UPDATE SET
              ...
              mode = EXCLUDED.mode,
              done_modules = EXCLUDED.done_modules,
